...
% evaluateCandidate  最大熵算法评价函数（仅熵+覆盖+通信，无信息素/前沿）
%
%  与 NewAlgorithm 的关键区别：
%    - 移除了 attractGain、frontierGain、排斥信息素相关项
%    - 通信权重固定不变（用于暴露通信退化问题）
    ...
    [lambdaC, lambdaH, lambdaR] = getDynamicWeights(rho, params);

    entropyGain = sum(localEntropy(validMask));
    repeatCost = sum(localVisit(validMask).^params.visitPenaltyPower) + revisitCount;
    ...
    if strcmpi(platformType, 'uav')
        ...
        score = lambdaC * coverageGain ...
              + lambdaH * entropyGain ...
              - lambdaR * repeatCost ...
              + params.relayWeightUAV * relayScore ...
              + params.spreadWeightUAV * spreadScore ...
              + params.smoothWeightUAV * smoothScore ...
        ...
        score = lambdaC * coverageGain ...
              ...
              + params.relayWeightUSV * relayScore ...
              + params.spreadWeightUSV * spreadScore ...
              + params.smoothWeightUSV * smoothScore ...
...
function [lambdaC, lambdaH, lambdaR] = getDynamicWeights(rho, params)
    rho = min(max(rho, 0), 1);
