...
%  截断历史并封装结果结构体，含通信质量指标（含8项新增指标）。
    ...
    % CARS 独有历史截断
    if isfield(history, 'healthHist')
        history.healthHist = history.healthHist(validIdx);
    end
    if isfield(history, 'etaBHist')
        history.etaBHist = history.etaBHist(validIdx);
    end
    if isfield(history, 'lambda2Hist')
        history.lambda2Hist = history.lambda2Hist(validIdx);
    end
    if isfield(history, 'rolesHist')
        history.rolesHist = history.rolesHist(1:stopInfo.stopStep, :);
    end
    ...
    % 使用 computeCommunicationMetrics 计算8项指标
